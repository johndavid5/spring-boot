...
public class MockServiceApplicationTest {

    @Autowired
    private MockMvc mockMvc;

    /**
    * Another useful approach is to not start the server at all, but test only the layer below
    ...
    */
    ...
    public void shouldReturnDefaultMessage() throws Exception {
        final String sWho = "MockServiceApplicationTest::shouldReturnDefaultMessage";
        System.out.println("*** " + sWho + "(): SHEMP: Gee, Moe, let's see if dha dummy (mock) soyvice retoyns a \"" + hello.HomeController.S_HELLO_WORLD + "\"...");
        
        this.mockMvc.perform(get("/")).andDo(print()).andExpect(status().isOk())
                .andExpect(content().string(containsString(
                        hello.HomeController.S_HELLO_WORLD)
                ));
    }
}